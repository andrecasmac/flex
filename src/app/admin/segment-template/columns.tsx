...
import { deleteSegmentById } from "@/da/Segments/segment-da";
...
export const columnsSegmentTemplate: ColumnDef<Segment>[] = [
  {
    ...
    cell: ({ row }) => {
      const segmentName: string =
        row.getValue("name"); /*Variable that stores the value of name*/
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const maxUse: number = parseInt(
        row.getValue("max")
      ); /*Variable that stores the value of maxUse*/
        ...
        <div className="flex justify-center">
          ...
          >
            <Button size="icon" variant={"ghost"}>
              <Pencil className="h-7 w-7 text-black dark:text-white " />
            </Button>
            {/*Button for the edit function*/}
          </Link>
        </div>
      );
    },
